...
void drawGeneticCircles() {
    ...
    for(int j=0;j<20;j++) {
        angle+=angleStep;
        glVertex3f(geneticCircles[best].x + cos(angle)*geneticCircles[best].radius ,
                   geneticCircles[best].y + sin(angle)*geneticCircles[best].radius, 0);
    }
    glEnd();
    glLineWidth(1);
}
...
{ 
    ...
    glutCreateWindow("genetic circle"); 
